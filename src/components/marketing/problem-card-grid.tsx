...
const cardOverlayStyles: CSSProperties[] = [
  {
    background:
    ...
    <div className={cn("grid gap-5 lg:grid-cols-3", className)}>
      {cards.map((card, index) => (
        <article
          key={card.title}
          className={cn(
            "section-card relative overflow-hidden p-6 lg:p-7",
            cardAccentClasses[index % cardAccentClasses.length],
          )}
          data-animate-item
        >
          <div
            ...
            style={cardOverlayStyles[index % cardOverlayStyles.length]}
          />
